...
- **2026-05-17T17:58:23-04:00:** Contoso Run 29 proved that benchmark-safe SSR postbacks need the full Razor Components form contract even when the page stays static SSR: `method="post"` alone is not enough. Add both `<AntiforgeryToken />` and `@formname` to benchmark forms or Playwright clicks will degrade into 400 responses and false runtime regressions.
- **2026-05-17T17:58:23-04:00:** The fastest stable repair path for Contoso data pages is: preserve BWFC `GridView`/`DetailsView`, inject preserved BLL services, and introduce typed page DTOs for row/detail projections instead of keeping `ItemType="object"`. That combination restored 40/40 in Run 29 while keeping L2 under 10 minutes.
...
- **2026-05-17T19:20:41-04:00:** Helper-class migration gaps need to run through `SourceFileCopier`, not just the page code-behind pipeline. Adding `ServerShimTransform` and `SelfInstantiationTransform` to the copied-source allow-list is what makes `Logic/` and `BLL/` fixes actually land in benchmark helper classes like `ExceptionUtility` and `ShoppingCartActions`.
- **2026-05-17T19:20:41-04:00:** `Server.MapPath` in non-page helpers is safest when the CLI rewrites only resolvable literal paths (direct string literals or local string variables) to `Path.Combine(AppContext.BaseDirectory, ...)`, adds `using System.IO;`, and leaves page classes on the existing `ServerShim` guidance path. That keeps helper utilities buildable without breaking the page-side shim contract.
- **2026-05-17T19:20:41-04:00:** Self-instantiation cleanup should be gated to DI-managed classes by constructor injection, `[Inject]`, or Logic/BLL service conventions. Once gated, the durable fix is to unwrap `using (var self = new CurrentClass())` and collapse `var self = new CurrentClass(); ... return self;` into `this`-based flow so benchmark helpers stop disposing or re-constructing their own scoped services.


≡ƒôî Team update (2026-05-30T15:46:42Z): 6 decisions merged from inbox including Executive Summary pattern, CLI fixes (4), helper transforms, L1 quality, SSR form contract — decided by Beast, Bishop, Forge
