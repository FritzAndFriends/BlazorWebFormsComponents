...
## 2026-03-24: Beast: EditorControls Tabbed Syntax Standardization
...
All EditorControls documentation files will standardize on **MkDocs tabbed syntax** (`=== "Web Forms" / === "Blazor"`) with **admonition callouts** for migration guidance.

### Rationale

1. **Improved UX for Migrating Developers** — Tabbed syntax renders as interactive tabs in MkDocs, allowing side-by-side Web Forms ↔ Blazor comparison without scrolling
2. **Pattern Consistency** — ValidationControls documentation (ControlToValidate.md) already demonstrates this pattern successfully
3. **Better Scannability** — Clear visual hierarchy: tabs group related content, admonitions highlight gotchas
4. **Reduced Cognitive Load** — Before/after structure is familiar to migrating developers

### What This Means

#### For Documentation Writers
- **New EditorControls docs:** Start with tabbed syntax from the beginning
- **Existing EditorControls docs:** Convert incrementally following the Button.md pattern (reference doc in this repo)
- **Code Examples:** Continue using `var` syntax (IDE0007 enforced as error in this repo)

#### For Other Control Categories
- **DataControls, ValidationControls, NavigationControls, LoginControls:** Not in scope for #510, but recommend same pattern for consistency
- **Migration Guides:** Already using tabbed syntax — maintain this standard
...
=== "Web Forms (Before)"
    ...
    ```html
    <asp:ControlName property="value" runat="server" />
    ```
...
!!! tip "Migration Tip"
    When converting, remember to remove `asp:` prefix and `runat="server"`.
```

**Key Points:**
- Each tab is indented 4 spaces after `===` line
- Code blocks are indented another 4 spaces (8 total)
- Admonitions use `!!! tip`, `!!! note`, `!!! warning`, `!!! danger`
- Admonitions follow tab groups but are NOT indented

### Files Converted (Issue #510)
28 EditorControls documentation files standardized to tabbed syntax in PR #514.

### Future Work
- Optionally extend pattern to other control categories for consistency
- Monitor MkDocs rendering to ensure tabs render correctly in all themes

### Sign-off
✅ All 28 EditorControls .md files converted and committed to squad/510-editor-controls-docs  
✅ PR #514 created against dev branch  
✅ Pattern documented for future writers

---

## 2026-03-24: Jubilee: DataControls + ValidationControls Tabbed Syntax Standardization

**Issues:** #505, #506, #507  
**Owner:** Jubilee (Sample Writer)  
**Date:** 2026-03-24  
**Status:** DECIDED  

### Decision

All DataControls and ValidationControls documentation files will standardize on **MkDocs tabbed syntax** (`=== "Web Forms" / === "Blazor"`) with expanded examples and migration guidance.

### Rationale

1. **Pattern Consistency** — Aligns with EditorControls and ValidationControls standardization
2. **Improved UX** — Interactive tabs for side-by-side comparison of Web Forms and Blazor syntax
3. **Complete Documentation** — Expanded stubs to production-ready content with code examples
4. **Developer Experience** — Consistent structure across all control categories

### Files Converted
- 20 DataControls .md files
- ValidationControls documentation
- All converted to tabbed syntax in PR #515

### Implementation
Same tabbed syntax template as EditorControls standardization, applied consistently across both control categories.

### Sign-off
✅ 20 DataControls + ValidationControls .md files converted  
✅ PR #515 created against dev branch  
✅ Stub documentation expanded to production quality

---

## 2026-03-24: Jubilee: AfterDepartmentPortal Runnable Demo Setup
...
## 2026-03-24: Forge: NuGet Static Asset Migration Strategy
