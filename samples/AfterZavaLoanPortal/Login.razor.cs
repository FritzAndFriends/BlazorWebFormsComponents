...
namespace ZavaLoanPortal
{
    ...
    private HyperLink lnkAuthGateway = default!;
    ...
    // AppSettings keys found: AuthGatewayLoginUrl
    // Add these to appsettings.json under "AppSettings" section or as top-level keys.
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            var returnUrl = (Request.QueryString["ReturnUrl"].ToString() ?? "/Default").Trim();
            ...
            var auth = ConfigurationManager.AppSettings["AuthGatewayLoginUrl"] ?? "http://localhost:8000/Login.aspx";
            lnkAuthGateway?.NavigateUrl = auth + "?ReturnUrl=" + WebUtility.UrlEncode(ResolveClientUrl(returnUrl));
            _lblMessage_Text = "Authentication is handled by ZavaAuthGateway.";
        }
    

    private object? _lblMessage_Text; // TODO: migrate from Web Forms code-behind
}
}
