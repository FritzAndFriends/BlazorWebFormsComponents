...
namespace WingtipToys
{
  public partial class ProductDetails
  {
    ...
    public IQueryable<Product> GetProduct(int maxRows, int startRowIndex, string sortByExpression, out int totalRowCount)
    {
      ...
      if (!productId.HasValue && int.TryParse(Request.QueryString["ProductID"], out var parsedProductId))
      {
      ...
      {
        query = query.Where(p => String.Compare(p.ProductName, productName, StringComparison.OrdinalIgnoreCase) == 0);
      }
      else
      {
