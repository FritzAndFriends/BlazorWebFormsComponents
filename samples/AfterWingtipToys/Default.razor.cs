...
namespace WingtipToys
{
  public partial class Default
  {
    ...
    private void Page_Error()
    {
      ...
      Exception exc = Server.GetLastError( /* TODO(bwfc-server): Server.GetLastError has NO SHIM. Use ILogger and middleware-based error handling (app.UseExceptionHandler). */ );

      // Handle specific exception.
      if (exc is InvalidOperationException)
      {
        // Pass the error on to the error page.
        Server.Transfer( /* TODO(bwfc-server): Server.Transfer has NO SHIM — architecturally incompatible with Blazor. Use NavigationManager.NavigateTo() instead. Server.Transfer does server-side URL rewriting which doesn't exist in Blazor. */ "ErrorPage.aspx?handler=Page_Error%20-%20Default.aspx",
            true);
      }
    }
  }
