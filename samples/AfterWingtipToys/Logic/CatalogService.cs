...
public class CatalogService
{
	...
	public CatalogService()
	{
		...
		{
			product.Category = _categories.Single(category => category.CategoryID == product.CategoryID);
		}

		foreach (var category in _categories)
		{
			category.Products = _products.Where(product => product.CategoryID == category.CategoryID).ToList();
		}
	}
	...
	public IReadOnlyList<Product> GetProducts(int? categoryId, string? categoryName)
	{
		...
		{
			products = products.Where(product => string.Equals(product.Category?.CategoryName, categoryName, StringComparison.OrdinalIgnoreCase));
		}
