...
public class NVPAPICaller
{
  ...
  public string HttpCall(string NvpRequest)
  {
    ...
    strPost = strPost + "&BUTTONSOURCE=" + WebUtility.UrlEncode(BNCode);
...
public sealed class NVPCodec : NameValueCollection
{
  ...
  public string Encode()
  {
    ...
    {
      string name = WebUtility.UrlEncode(kv);
      string value = WebUtility.UrlEncode(this[kv]);
      if (!firstPair)
      {
        sb.Append(AMPERSAND);
      }
  ...
  public void Decode(string nvpstring)
  {
    ...
    foreach (string nvp in nvpstring.Split(AMPERSAND_CHAR_ARRAY))
    {
      ...
      if (tokens.Length >= 2)
      {
        string name = WebUtility.UrlDecode(tokens[0]);
        string value = WebUtility.UrlDecode(tokens[1]);
        Add(name, value);
      }
    }
  }
