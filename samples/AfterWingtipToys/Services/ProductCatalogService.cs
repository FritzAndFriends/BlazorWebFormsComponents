...
public class ProductCatalogService
{
    ...
    public IReadOnlyList<Product> GetProducts(int? categoryId, string? categoryName)
    {
        ...
        {
            query = query.Where(p => string.Equals(p.Category?.CategoryName, categoryName, StringComparison.OrdinalIgnoreCase));
        }
    ...
    public Product? GetProduct(int? productId, string? productName)
    {
        ...
        {
            return _products.FirstOrDefault(p => p.ProductID == productId.Value);
        }

        if (!string.IsNullOrWhiteSpace(productName))
        {
            return _products.FirstOrDefault(p => string.Equals(p.ProductName, productName, StringComparison.OrdinalIgnoreCase));
        }
