...
ComponentBase                          ← Blazor framework
  └── BaseWebFormsComponent            ← BWFC base: ID, Enabled, Visible, lifecycle events
        └── BaseStyledComponent        ← Style props: BackColor, CssClass, Font, etc. (10 props)
...
4. **How to obtain these counts:**
   - **Preferred:** Write a .NET Framework 4.8 console app that uses reflection (`Type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)`) on each control type. This app lives in `tools/WebFormsPropertyCounter/` and produces a JSON baseline file.
   - **Acceptable fallback:** Manually count from the [.NET Framework 4.8 API reference](https://learn.microsoft.com/en-us/dotnet/api/system.web.ui.webcontrols) documentation, documented in a versioned JSON file.
...
{
  "version": "1.0",
  "source": "System.Web 4.8 reflection via tools/WebFormsPropertyCounter",
...
```
dev-docs/reference-baselines.json     ← Versioned baseline data
tools/WebFormsPropertyCounter/        ← .NET Fx 4.8 console app to generate baselines
```

The baselines JSON file is checked into the repo and loaded at runtime by `ComponentHealthService`. It can be regenerated by running the counter tool against a .NET Framework 4.8 installation.

If the counter tool can't run (e.g., no .NET Fx 4.8 SDK available), the baselines can be manually curated from MSDN documentation. The JSON file has a `source` field that documents which method was used.
...
**What happened:** Using `GetProperties(BindingFlags.Public | BindingFlags.Instance)` without `DeclaredOnly` pulled ALL inherited [Parameter] properties from BaseWebFormsComponent (20), BaseStyledComponent (10), etc. Button showed "40 implemented" instead of ~7.
...
9. **All 52 completed components** show at least tests=✅ (797+ bUnit tests exist)
...
### BaseWebFormsComponent (20 parameters)
...
### BaseStyledComponent (10 parameters)
